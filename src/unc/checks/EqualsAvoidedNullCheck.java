...
import unc.symbolTable.IdentifierType;
 ...
 */
@FileStatefulCheck
public class EqualsAvoidedNullCheck extends UNCCheck {
...
//    public static final String MSG_EQUALS_IGNORE_CASE_AVOID_NULL = "equalsIgnoreCase.avoid.null";
    public static final String MSG_KEY_INFO = "equalsAvoidedNull";
    public static final String MSG_KEY_WARNING = "equalsDidNotAvoidNull";
    ...
    @Override
    protected void doVisitToken(final DetailAST ast) {
        switch (ast.getType()) {
...
//    private boolean checkMethodCall(DetailAST methodCall) {
...
//        return false;
//      }
//      IdentifierType anIndentifierType = getCalledOnStringType(objCalledOn);
//      if (anIndentifierType == IdentifierType.UNTYPED || anIndentifierType == IdentifierType.METHOD) {
//        return false;
//      }
//      if (anIndentifierType == IdentifierType.CONSTANT && isInfo() ) {
//        log(methodCall, MSG_KEY_INFO);
//      } else if (anIdentifierType == IdentifierType)
//     
//          if (EQUALS.equals(methodName)) {
//              log(methodCall, MSG_KEY_INFO);
//          }
...
//      if (is ( checkMethodCallWarning(methodCall)) {
//        return;
//      };
//     checkMethodCallInfo(methodCall);
      if (isInfo()) {
        checkMethodCallInfo(methodCall);
      } else {
        checkMethodCallWarning(methodCall);
      }
    }
    /**
     ...
     */
    private boolean checkMethodCallInfo(DetailAST methodCall) {
        DetailAST objCalledOn = methodCall.getFirstChild().getFirstChild();
        if (objCalledOn.getType() == TokenTypes.DOT) {
            objCalledOn = objCalledOn.getLastChild();
        }
...
//                && expressionIsStringVariable(expr)
    ...
    private boolean checkMethodCallWarning(DetailAST methodCall) {
      DetailAST objCalledOn = methodCall.getFirstChild().getFirstChild();
      if (objCalledOn.getType() == TokenTypes.DOT) {
          objCalledOn = objCalledOn.getLastChild();
      }
...
//              && expressionIsStringConstant(expr)
    ...
    private  boolean expressionIsStringConstant(final DetailAST expr) {
      DetailAST arg = expr.getFirstChild();
      arg = skipVariableAssign(arg);
...
//          argIsNotNull = arg.getType() == TokenTypes.STRING_LITERAL;
     ...
     */
    protected IdentifierType getCalledOnStringFieldOrVariable(DetailAST objCalledOn) {
        IdentifierType  result = IdentifierType.UNTYPED;
        final DetailAST previousSiblingAst = objCalledOn.getPreviousSibling();
        if (previousSiblingAst == null) {
            result = getStringVariableType(objCalledOn);
        }
        else {
            if (previousSiblingAst.getType() == TokenTypes.LITERAL_THIS) {
                result = getStringFieldOrVariableFromThisInstance(objCalledOn);
            }
            else {
                final String className = previousSiblingAst.getText();
                result = getStringFieldOrVariableFromClass(objCalledOn, className);
            }
        }
        return result;
    }
    
    protected boolean isCalledOnStringConstant(DetailAST objCalledOn) {
     
      if ( objCalledOn.getType() == TokenTypes.STRING_LITERAL) return true;
      IdentifierType anIdentifierType = getCalledOnStringFieldOrVariable(objCalledOn);
      return (anIdentifierType == IdentifierType.CONSTANT);      
    
  }
    protected IdentifierType getCalledOnStringType(DetailAST objCalledOn) {
      
      if ( objCalledOn.getType() == TokenTypes.STRING_LITERAL) return IdentifierType.CONSTANT; // not really an id
      return getCalledOnStringFieldOrVariable(objCalledOn);    
    
  }
    protected boolean isCalledOnStringVariable(DetailAST objCalledOn) {
      
      if ( objCalledOn.getType() == TokenTypes.STRING_LITERAL) return false;
      IdentifierType anIdentifierType = getCalledOnStringFieldOrVariable(objCalledOn);
      return (anIdentifierType == IdentifierType.VARIABLE);      
    
  }

    public static IdentifierType getIdentifierType (DetailAST varDefAST) {
      final DetailAST modifiersAST =
              varDefAST.findFirstToken(TokenTypes.MODIFIERS);
      if (modifiersAST.branchContains(TokenTypes.FINAL) && 
              varDefAST.branchContains(TokenTypes.STRING_LITERAL)) {
     ...
     */
    private IdentifierType getStringVariableType(DetailAST objCalledOn) {
      IdentifierType result = IdentifierType.UNTYPED;
        ...
        while (frame != null) {
            ...
            if (field != null
                ...
                if (STRING.equals(getFieldType(field))) {
                  result = getIdentifierType(field);
                };
     ...
     */
    private IdentifierType getStringFieldOrVariableFromThisInstance(DetailAST objCalledOn) {
//        IdentifierType result = IdentifierType.UNTYPED;
        final String name = objCalledOn.getText();
        final DetailAST field = getObjectFrame(currentFrame).findField(name);
        if ( STRING.equals(getFieldType(field))) {
          return getIdentifierType(field);
        }
        return  IdentifierType.UNTYPED;
    }
     ...
     */
    private IdentifierType getStringFieldOrVariableFromClass(DetailAST objCalledOn,
...
//
    ...
    protected String msgKey() {
      return isInfo()?MSG_KEY_INFO:MSG_KEY_WARNING;
    }
